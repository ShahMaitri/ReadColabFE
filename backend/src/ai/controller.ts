...
export class AIController {
  ...
  /**
   * Get book recommendations based on user preferences (AI-powered)
  ...
  /**
   * Get mood-based book recommendations from database (no rate limit)
   * Generated by GitHub Copilot
   * POST /ai/mood-recommendations
   * Body: { mood: string }
   */
  moodRecommendations = asyncHandler(async (req: Request, res: Response) => {
    const { mood } = req.body;

    if (!mood || typeof mood !== 'string') {
      throw new AppError('Mood string is required', 400);
    }

    if (mood.trim().length === 0) {
      throw new AppError('Mood cannot be empty', 400);
    }

    const { bookRepository } = await import('../repositories');
    const books = await bookRepository.findByMood(mood.toLowerCase(), 7);

    if (books.length === 0) {
      return res.status(200).json({
        success: true,
        message: 'No books found for this mood',
        data: {
          recommendations: [
            {
              title: 'No books available',
              author: 'Read Colab Library',
              reason: `Sorry, we don't have books matching your "${mood}" mood right now. Please check our catalog or try another mood!`
            }
          ]
        }
      });
    ...
    const recommendations = books.map((book: any) => ({
      title: book.title,
      author: book.author,
      ...
      cover: book.cover,
      id: book.id,
      reason: `This ${book.category} book perfectly matches your "${mood}" mood!`
    }));

    res.status(200).json({
      success: true,
      message: 'Mood-based recommendations from library database',
      data: {
        recommendations,
        source: 'database'
      }
